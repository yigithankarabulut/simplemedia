...
func (h *Handler) DeclineFriends(c *fiber.Ctx) error {
	var (
	...
	if err := h.service.DeclineFriends(c.Context(), friend); err != nil {
		return c.JSON(h.util.BasicError(err, fiber.StatusInternalServerError))
	}
	return c.JSON(h.util.Response(fiber.StatusOK, constant.FriendRequestDecline))
}
